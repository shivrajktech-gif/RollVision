...
from .models import Faculty, SystemSettings, Student, FaceEncoding, AttendanceRecord, AttendanceSession, Division
from .forms import FacultyForm, SettingsForm, StudentForm
...
def train_system_background():
    """
    Background task to retrain the face recognition model
    """
    try:
        # Import here to avoid circular imports if placed elsewhere
        from .models import Student
        
        # Get all students with photos
        # We start with a delay or lock? No, LBPH training is fast for small sets.
        # But we valid data.
        ...
        if training_data:
            ...
            # We assume if they were in the training set, they are "trained"
            # But simpler to just mark them True if training succeeded.
            # However, Student model update inside thread might be risky with connections?
            # Django handles DB in threads usually fine if connections are managed.
            # For simplicity, we won't bulk update here to avoid race conditions.
            
    except Exception as e:
        logger.error(f"Background training failed: {e}")

def trigger_training():
    """Fire and forget training task"""
    t = threading.Thread(target=train_system_background)
    t.daemon = True
    t.start()
...
def students(request):
    ...
    if request.method == "POST":
        ...
        face_data = request.POST.get('face_data', '').strip()  # Base64 from camera
            ...
            with transaction.atomic():  # ALL OR NOTHING!
                ...
                if face_data:
                    try:
                        ...
                        if not success:
                            ...
                            # Note: encode_face might verify quality or return dlib encoding
                            # For LBPH, we just need the image saved.
                            image_path = face_recognizer.save_face_image(image, student_id, face_rect)
                            
                            # CRITICAL: Save the cropped face as the student's main photo
                            # This ensures consistency for training
                            student.photo = image_path
                            student.is_trained = True  # optimistically set to True (will be trained shortly)
                            student.save()
                            
                            # Trigger training
                            trigger_training()
                            
                            # Create a record for history (optional, or with dummy data)
                            FaceEncoding.objects.create(
                                student=student,
                                encoding_data=json.dumps({'version': 'lbph_v1'}), # Dummy valid JSON
                                image_path=image_path,
                                is_active=True
                            )
                            
                            logger.info(f"✅ Student {student_id} registered and queued for training.")
                            face_success = True
                ...
                # PHOTO UPLOAD METHOD
                    ...
                    try:
                        ...
                        # Convert to numpy array (ensure uint8 format)
                        import numpy as np
                        ...
                        if not success:
                            ...
                            FaceEncoding.objects.create(
                                student=student,
                                encoding_data=json.dumps({'version': 'lbph_v1'}),
                                image_path=image_path,
                                is_active=True
                            )
                            
                            logger.info(f"✅ Student {student_id} registered (upload) and queued for training.")
                            face_success = True
                ...
                elif face_error_message == "Recognition unavailable (System Library Error)":
                     messages.warning(request, f"⚠️ Student {name} registered successfully, but face recognition features are unavailable due to server configuration.")
...
def reports(request):
    ...
    # Add class years with selected flags to avoid template comparison issues
    class_filter = context.get('class_filter', '')
    context['class_years'] = [
        {'value': 'FE', 'label': 'FE', 'selected': class_filter == 'FE'},
        {'value': 'SE', 'label': 'SE', 'selected': class_filter == 'SE'},
        {'value': 'TE', 'label': 'TE', 'selected': class_filter == 'TE'},
        {'value': 'BE', 'label': 'BE', 'selected': class_filter == 'BE'},
    ]
    
    # Add divisions with selected flags
    division_filter = context.get('division_filter')
    context['divisions_list'] = [
        {'id': d.id, 'name': d.name, 'selected': division_filter == d.id}
    ...
    if request.method == "POST" and request.POST.get("form_type") == "feedback":
        ...
        if feedback:
            send_mail(
                ...
                recipient_list=["shivraj.rollvision@gmail.com"],
                fail_silently=False,
            )
...
def process_attendance(request):
    """API endpoint to process face recognition for attendance (Updated to use LBPH)"""
    logger.info("Attendance processing requested (Legacy/Stand-alone)")
    if request.method == 'POST':
        try:
            data = json.loads(request.body)
            ...
            # LBPH detection (No on-the-fly training needed)
            ...
            for result in results:
                ...
                # Get student
                try:
                    student = Student.objects.get(student_id=student_id) # LBPH map uses student_id string usually? 
                    # Wait, in face_utils.py: self.student_map = {int(k): v ...} where v is student_id string. 
                    # So result['student_id'] is the string ID (e.g., "STD001").
                except Student.DoesNotExist:
                    logger.warning(f"Recognized ID {student_id} not found in DB")
                    continue
                ...
                if existing_record:
                    ...
                    # Mark present (Stand-alone mode, no session??)
                    # If this view is used without a session, we might lack Session FK.
                    # But AttendanceRecord might require it?
                    # Let's check models.py... implied it's optional or handled.
                    # For now, we create it.
                    try:
                        attendance = AttendanceRecord.objects.create(
                            ...
                            date=today
                            # Session field is nullable? We assume so for this legacy view.
                        )
            ...
            if not processed_students:
                return JsonResponse({
                    'success': False,
                    'message': f'Detected {len(results)} face(s), but none were recognized.',
